...
function Login() {
  ...
  useEffect(() => {
    if (loginResult === "SUCCESS") {
      // Retrieve sessionId from location.state or sessionStorage
      // const sessionId =
      //   location.state.sessionId || sessionStorage.getItem("currentSessionId");
      ...
      if (sessionId) {
        axios
          .put("http://localhost:5000/api/teacher/mark-attendance", {
            sessionId,
            studentId: tempAccount.id,
            present: true,
          })
          .then(() => {
        ...
        console.error("sessionId is not provided");
      }
      ...
      if (counter === 0) {
        ...
        navigate("/teacher-dashboard", { replace: true,state: { initialTab: 3 } });
      }
  ...
  async function loadLabeledImages() {
    ...
    if (detections) {
      console.log("User detections : ", detections);
    ...
    <div className="h-full flex flex-col items-center justify-center gap-[24px] max-w-[720px] mx-auto">
      {!localUserStream && !modelsLoaded && (
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
          <span className="block">
            You're Attempting to Mark Attendance With Your Face.
          </span>
      ...
      {!localUserStream && modelsLoaded && (
        <h2 className="text-center text-3xl font-extrabold tracking-tight text-gray-900 sm:text-4xl">
          <span className="block text-indigo-600 mt-2">
            Please Recognize Your Face to Completely Mark Attendance.
          </span>
        </h2>
      )}
